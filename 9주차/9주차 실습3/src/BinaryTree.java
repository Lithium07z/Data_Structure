...
public class BinaryTree<Key extends Comparable<Key>> {
	...
	public LinkedList<Node> subTreeCompare(BinaryTree t) {
		LinkedList<Node> list1 = new LinkedList<Node>(); // tA 트리의 노드를 저장할 리스트
		LinkedList<Node> list2 = new LinkedList<Node>(); // tB 트리의 노드를 저장할 리스트
		LinkedList<Node> list3 = new LinkedList<Node>(); // tA와 tB트리의 겹치는 노드 중 tA의 노드만 저장하는 배열 
		LinkedList<Node> list4 = new LinkedList<Node>(); // tA와 tB트리의 겹치는 노드 중 tB의 노드만 저장하는 배열 
		LinkedList<Node> list5 = new LinkedList<Node>(); // 일치하는 서브트리가 없는 경우 반환할 리스트
		Node tA = this.root; // tA트리의 루트노드를 저장
		Node tB = t.root; // tB트리의 루트노드를 저장 
		
		addNodeToList(tA, list1); // 리스트에 tA 트리의 노드를 저장 
		addNodeToList(tB, list2); // 리스트에 tB 트리의 노드를 저장 
		for (int i = 0; i < list1.size(); i++) { // 두 트리에 같은 노드가 있는지 검사 
			for (int j = 0; j < list2.size(); j++) {
				if (isEqual(list1.get(i), list2.get(j))) { // 같은 노드가 있다면 
					list3.add(list1.get(i)); // tA트리의 노드는 list3에 저장 
					list4.add(list2.get(j)); // tB트리의 노드는 list4에 저장 
				}
			}
		}
		
		if(isEqual(list3.getFirst(), list4.getFirst())) { // 겹치는 노드들의 탐색결과를 비교
			return list3; // tA트리에서 호출했으므로 tA트리의 노드를 담은 list를 반환 
		}
		list5.add(new Node(null, null, null));
		return list5; // 일치하는 서브트리가 없는 경우 
	}
	
	public void addNodeToList(Node tRoot, LinkedList list) { // 레벨 순회방식으로 리스트에 노드를 추가 
		Queue<Node> q = new LinkedList<Node>(); // 큐 이용
		Node t;
		q.add(tRoot); // 루트 노드를 큐에 삽입
		while (!q.isEmpty()) {
			t = q.remove(); // 큐 가장 앞에있는 노드 제거
			list.add(t); // 리스트에 노드 추가 
			if (t.getLeft() != null) { // 제거된 왼쪽 자식이 null이 아니면
				q.add(t.getLeft()); // 큐에 왼쪽 자식 삽입
			}
			if (t.getRight() != null) { // 제거된 오른쪽 자식이 null이 아니면
